...
title_grob <- textGrob("Вплив вітру на концентрацію частинок PM2.5 і PM10", gp = gpar(fontsize = 14))
x_grob <- textGrob("Швидкість вітру (м/с)", gp = gpar(fontsize = 14))
y_grob <- textGrob("Концентрація частинок", gp = gpar(fontsize = 14), rot=90)

windspeed_vs_pm <- grid.arrange(
  grobs = list(windspeed_vs_pm2.5, windspeed_vs_pm10),
  top = title_grob,
  bottom = x_grob,
  left = y_grob,
  ncol = 2
...
ggsave(here("plots", "question1", "wind_speed_vs_pm.png"), windspeed_vs_pm, dpi = 200)
```
...
aqi_pm10_diagram <- ggplot(df, aes(x = pm10, y = aqi)) +
    geom_point(alpha = 0.5, color = "blue") +
    geom_smooth(method = "lm", col = "red") +
    labs(title = "Залежність AQI від PM10")
...
ggsave(here("plots", "question1", "scatter_pm2_5_region.png"), scatter_pm2_5_region, dpi = 200, bg = "white")
```
...
ggsave(here("plots", "question1", "scatter_pm10_region.png"), scatter_pm10_region, dpi = 200, bg = "white")
```

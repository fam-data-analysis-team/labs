...
ggsave(here("plots", "outliers", "count-bar.png"), p, width = 15)
```
...
p <- df %>%
  ...
  geom_point() +
  geom_hline(aes(yintercept = to), colour = "red", lwd = 1.5) + # Позначимо лінії відсічення
  geom_hline(aes(yintercept = bo), colour = "red", lwd = 1.5) +
  labs(x = "Дата", y = "Значення")

ggsave(here("plots", "outliers", "scatter.png"), p, width = 15, height = 15)
```

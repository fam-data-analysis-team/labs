...
dir.create(here("plots", "question3", "county-box"), recursive = TRUE, showWarnings = FALSE)
```
...
ggsave(here("plots", "question3", "box.png"), p, bg = "white")
```
...
for (i in unique(df_daytime$county)) {
  p <- df_daytime %>%
      ...
      ggplot(aes(x = daytime, y = log(aqi))) +
        ...
        labs(
          x = "Інтервал часу (год)",
          title = sprintf("Зміна aqi впродовж доби (%s)", i)
        )
  
  ggsave(here("plots", "question3", "county-box", paste(i, ".png", sep = "")), p, bg = "white")
}
```
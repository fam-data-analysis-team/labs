...
latex_table_1 <- kable(results_table, 
                       format = "latex", 
                       booktabs = TRUE,
                       caption = "Порівняння методів ядерної регресії",
...
comparison_regression_plot <- ggplot(df_all_plot, aes(x = .data[[x_col_plot]], y = .data[[y_col_plot]])) +
  ...
  labs(title = paste("Порівняння методів ядерної регресії: AQI від", x_col_plot))
  theme_minimal() +
...
ggsave(file.path(plots_path, "npreg_comparison_final.png"),

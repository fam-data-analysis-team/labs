...
Статистики, що мають асимптотично нормальний розподiл, для якого асимптотичну дисперсiю важко оцiнити.
...
# --- Збереження в CSV ---
write_csv(results, if (test_mode) "CI_spearman_bootstrap.csv" else "spearman_bootstrap_intervals_full.csv")

# --- Markdown-таблиця всіх результатів ---
if (nrow(results) > 0) {
  kable(
    results %>%
    ...
    caption = "Бутстреп-довірчі інтервали для коефіцієнтів Спірмана",
    format = "markdown"
  )
} else {
  cat("**Жодна пара змінних не пройшла фільтр мінімальної кількості спостережень.**")
}
